...
# NPU adaptation for Ascend devices
...
def test_flash_attn_varlen_block_output(
    ...
    if p_dropout > 0.0:
        ...
        # Note: convert_flash_attn_S_to_softmax uses CUDA-specific logic
        # (_get_block_size -> torch.cuda.get_device_capability).
        # On NPU, skip the detailed attention matrix conversion and use
        # a simpler dropout mask derived from S_dmask directly.
        try:
            ...
            print(f"Skipping detailed dropout verification on NPU: {e}")
            dropout_mask = None
    else:
        dropout_mask = None
